                    ...
                    uint64_t to_timestampMicroseconds)
{
  for (auto it_k_minus_1 = imuMeasurementDeque_.begin();
    ...
    arp::kinematics::ImuKinematicsJacobian F;
    arp::kinematics::Imu::stateTransition(x_, z_k_minus_1, z_k, x_, &F);    // x_k = f(x_k_minus_1)
    ...
    for(int k=0; k < 3; k++){
      Noise(k, k) = 0;
      Noise(k+3, k+3) = delta_t*pow(sigma_c_gyr_, 2);
      Noise(k+6, k+6) = delta_t*pow(sigma_c_acc_, 2);
      Noise(k+9, k+9) = delta_t*pow(sigma_c_gw_, 2);
      Noise(k+12, k+12) = delta_t*pow(sigma_c_aw_, 2);
    }
                   ...
                   int id, unsigned int p)
{
  ...
  // pose T_WC: transforms points represented in camera coordinates
  // into World coordinates
  //kinematics::Transformation T_WC(x_.r_W, x_.q_WC);
  ...
  Eigen::Vector3d p_W(hp_W[0], hp_W[1], hp_W[2]);
  ...
  const Eigen::Vector4d hp_C = T_SC_.inverse()*T_WS.inverse() * hp_W;
  Eigen::Vector3d p_C(hp_C[0], hp_C[1], hp_C[2]);
  // TODO: calculate the reprojection error y (residual)
  // using the PinholeCamera::project
  Eigen::Matrix<double, 2, 3> U;
  Eigen::Vector2d projC;
  cameras::CameraBase::ProjectionStatus test = cameraModel_.project(p_C, &projC, &U);

  const Eigen::Vector2d y = imagePointMeasurements.col(p) - projC;
  // TODO: check validity of projection -- return false if not successful!
  if(test == cameras::CameraBase::ProjectionStatus::Successful){
  // TODO: calculate measurement Jacobian H

  Eigen::Matrix<double, 2, 9> zeros;
  zeros.setZero(2, 9);
  Eigen::Matrix<double, 3, 3> C_ws = T_WS.C();
  Eigen::Matrix<double, 3, 3> C_cs = T_SC_.inverse().C();

  Eigen::Matrix<double, 3, 3> dr = -1 * C_ws.transpose();
  Eigen::Matrix<double, 3, 3> dalpha =
  C_ws.transpose() *arp::kinematics::crossMx(Eigen::Vector3d(hp_W.head(3) - x_.r_W));
  Eigen::Matrix<double, 2, 15> H;
  H.block<2, 3>(0, 0) = U * C_cs * dr;
  H.block<2, 3>(0, 3) = U * C_cs * dalpha;
  H.block<2, 9>(0, 6) = zeros;
  ...
  Eigen::VectorXd Delta_chi(15);
  Delta_chi = K*y;
  std::cout << "Delta_chi" << std::endl;
  // TODO: perform update. Note: multiplicative for the quaternion!!

  x_.r_W += Delta_chi.segment(0, 3);  ///< The position relative to the W frame.
  x_.q_WS.coeffs() = arp::kinematics::oplus(x_.q_WS) *  arp::kinematics::deltaQ(Delta_chi.segment<3>(3)).coeffs();  ///< The quaternion of rotation W-S.
  x_.v_W += Delta_chi.segment(6, 3);  ///< The velocity expressed in W frame.
  x_.b_g += Delta_chi.segment(9, 3);  ///< The gyro bias.
  x_.b_a += Delta_chi.segment(12, 3);  ///< The accelerometer bias.
  std::cout << "r_W: " << x_.r_W << std::endl;

  // TODO: update to covariance matrix:
  Eigen::Matrix<double, 15, 15> identity; identity.setIdentity();
  P_ = (identity - K*H)*P_;
