...
Autopilot::DroneStatus Autopilot::droneBattery()
{
...
bool Autopilot::move(double forward, double left, double up,
                     double rotateLeft)
{
  if(forward < -1 || forward > 1) return false;
  if(left < -1 || left > 1) return false;
  if(up < -1 || up > 1) return false;
  if(rotateLeft < -1 || rotateLeft > 1) return false;

  // TODO: implement...
   geometry_msgs::Twist message;
   message.linear.x = forward;
   message.linear.y = left;
   message.linear.z = up;
   message.angular.z = rotateLeft;

   pubMove_.publish(message);
